...
async function loadFrozenTraining(dataRoot, config) {
  const buffers = {};
...
async function main() {
  ...
  const holdout = config.subjectVerbHoldout;
  if (!holdout?.seed || !holdout.modulo || !holdout.documents || !holdout.sentences) {
    throw new Error('Frozen subject–verb holdout configuration is incomplete');
  }
  const buffers = await loadFrozenTraining(dataRoot, config);
  ...
  if (holdoutIndexes.length !== holdout.sentences || holdoutDocuments.size !== holdout.documents) {
    throw new Error('Frozen subject–verb holdout assignment mismatch');
  }
  ...
  let goldVerbFormEdits = 0;
  for (const { index } of holdoutIndexes) {
    goldVerbFormEdits += gold[index].edits.filter((edit) => edit.type === 'R:VERB:FORM').length;
  ...
  const report = {
    ...
    warning: 'This precision-first detector covers only explicit adjacent pronouns and closed auxiliary/modal paradigms. Neutral target distance is not automatically harmful because human target hypotheses are not exhaustive.',
  };
...
main().catch((error) => {
  console.error(`[benchmark:subject-verb:holdout] ${error.message}`);
  process.exitCode = 1;
});
